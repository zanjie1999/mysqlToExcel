...
if (!empty($dbname) && !empty($host) && !empty($port) && !empty($user) && !empty($pwd)) {
    ...
    <body>
        <form action="" method="post">
            数据库名: <input name="dbname" value="hlw"><br>
            数据库地址: <input name="host" value="120.24.185.249"><br>
            数据库端口: <input name="port" value="3306"><br>
            数据库用户名: <input name="user" value="Gzepro"><br>
            数据库密码: <input name="pwd" value="Gzepro123"><br>
            自定义查询sql (默认导出所有表结构): <br>
            <textarea name="inpsql" cols="40" rows="5"></textarea><br>
            <input type="submit" value="下载 sql.slx">
        </form>
    </body>
    </html>
    ');
